...
import { fetchUser, createUser } from "@/utils/authAPI";
...
export const SignInButton = () => {
  ...
  // todo this logic should probbaly be in hook
  useEffect(() => {
    async function checkUser() {
      if (session?.user.email && session?.user.name) {
        const currentUser = await fetchUser(session.user.email);
        if (!currentUser) {
          const response = createUser(session.user.email, session.user.name);
        }
        setUserChecked(true);
      }
    }
